...
note(TC, Text) -> unwrap_unit(hegel_native:note(TC, unicode:characters_to_binary(Text))).
event(Name) -> unwrap_unit(hegel_native:event(current_test_case(), unicode:characters_to_binary(to_text(Name)))).
event(Name, Value) when is_number(Value) ->
    unwrap_unit(hegel_native:event_value(current_test_case(), unicode:characters_to_binary(to_text(Name)), erlang:float(Value)));
...
target(Name, Value) when is_number(Value) ->
    unwrap_unit(hegel_native:target(current_test_case(), unicode:characters_to_binary(to_text(Name)), erlang:float(Value))).
